 ...
 * invariant — that bean cannot satisfy {@link AnalyticsEventBridge}'s
 * constructor, which asks for {@code KafkaTemplate<String,
 * ResponseRecorded>}. Same fix identity-service's own KafkaConfig uses:
 * one explicitly typed bean, everything else about the producer still
 * comes from {@code spring.kafka.*}.
 */
@Configuration

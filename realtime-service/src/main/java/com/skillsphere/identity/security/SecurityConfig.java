 ...
 *
 * <p>{@code /ws/**} stays open at the transport level, matching the
 * monolith's own rule for it: the WebSocket handshake is itself a plain
 * HTTP request, and arenas allow unauthenticated guest participants — the
 * real identity check happens inside the STOMP CONNECT frame
 * ({@code realtime.internal.StompAuthChannelInterceptor}), the one place that can tell a
 * signed-in learner from a guest without rejecting the guest outright.
 * Preview, join and answer are also public for the same reason (a guest
 * has no bearer token to present); create/start/advance/end are not
 * listed and stay authenticated by the default rule, since only the
 * instructor who owns the arena may drive it.
 */
@Configuration
    ...
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
        http
                ...
                .authorizeHttpRequests(auth -> auth
                        ...
                        // Default deny. create/start/advance/end fall
                        // through to here — any signed-in user, no role
                        // gate beyond authentication, matching the
                        // monolith's own rule.
